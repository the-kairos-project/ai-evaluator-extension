import { Button, Dialog, FormField, Heading, Input, Select, Switch } from '@airtable/blocks/ui';
...
export const SettingsDialog = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const [rankingKeyword, setRankingKeyword] = useState(ACADEMIC_TEMPLATE.rankingKeyword);
  ...
  useEffect(() => {
    ...
    const storedShowDetailedHelp = globalConfig.get('showDetailedHelp') as boolean || false;
          ...
          <div className="mb-3">
            <FormField label="Template" className="mb-2">
              <Select
                ...
                onChange={value => {
                  if (value === 'custom') {
                    const template = AVAILABLE_TEMPLATES.find(t => t.id === selectedTemplate) || ACADEMIC_TEMPLATE;
              ...
              />
              <div className="mt-1 text-xs text-gray-500">
                {customPrompt ? 'Using custom template' : 'Using standard academic template'}
              </div>
            </FormField>
            ...
            {customPrompt && (
              <FormField label="Custom Prompt Template" className="mb-2">
                ...
                />
                <div className="mt-1 text-xs text-gray-500">
                  <div className="font-medium mb-1">Required placeholders (automatically replaced):</div>
                  <div>• <code>{'{criteriaString}'}</code> → Your evaluation criteria from field configuration</div>
                  <div>• <code>{'{rankingKeyword}'}</code> → The keyword for final score (e.g., FINAL_RANKING, SCORE)</div>
                  <div>• <code>{'{additionalInstructions}'}</code> → Extra instructions from field below (optional)</div>
                </div>
              </FormField>
            )}
            ...
            <FormField label="Ranking Keyword" className="mb-2">
              ...
              />
              <div className="mt-1 text-xs text-gray-500">
                The keyword the AI should use to output the final score (e.g., FINAL_RANKING, SCORE, RATING)
              </div>
            </FormField>
          ...
          <div className="mb-3">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="text-xs text-gray-500">Display expanded guidance under all configuration fields</div>
              </div>
              <Switch
            ...
            <FormField label="API Concurrency" className="mb-2">
              <Select
                ...
                onChange={value => setApiConcurrency(parseInt(value as string))}
              />
              <div className="mt-1 text-xs text-gray-500">
                Number of simultaneous API calls. Higher values are faster but may hit rate limits.
              </div>
            </FormField>
          </div>
        </FormField>
