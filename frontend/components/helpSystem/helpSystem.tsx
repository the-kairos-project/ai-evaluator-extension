...
import { FormField, Icon } from '@airtable/blocks/ui';
import React, { useState } from 'react';
import { HELP_CONTENT, type HelpContent } from './helpContent';
      ...
      {isVisible && (
        <div
          className={`absolute z-50 w-80 p-3 bg-white border rounded-lg shadow-lg ${
            isLogsField || isApplicantField ? 'bottom-6 left-6' : 'top-6 left-6'
          }`}
          style={{
            maxWidth: '320px',
            ...
            ...(isLogsField || isApplicantField
              ...
              : {
                  ...
                  transform:
                    buttonRef.current &&
                    buttonRef.current.getBoundingClientRect().left + 320 >
                      window.innerWidth
                      ? 'translateX(-280px)'
                      : 'translateX(0)',
                }),
        ...
        >
          <div className="text-sm">
            <div className="font-medium text-gray-900 mb-2">{content.purpose}</div>
              ...
              <div className="mb-2">
                ...
                <ul className="mt-1 ml-4 list-disc">
                  {content.examples.map((example) => (
                    <li key={example} className="text-gray-600 text-xs">
                      {example}
                    </li>
                  ))}
                </ul>
              ...
              <div className="mb-2">
                ...
                <ul className="mt-1 ml-4 list-disc">
                  {content.bestPractices.map((practice) => (
                    <li key={practice} className="text-gray-600 text-xs">
                      {practice}
                    </li>
                  ))}
                </ul>
      ...
      <div className="text-sm">
        ...
        {content.examples && content.examples.length > 0 && (
          <div className="mb-2">
            ...
            <ul className="mt-1 ml-4 list-disc">
              {content.examples.map((example) => (
                <li key={example} className="text-blue-700 text-xs">
                  {example}
                </li>
              ))}
            </ul>
        ...
        {content.bestPractices && content.bestPractices.length > 0 && (
          <div className="mb-2">
            ...
            <ul className="mt-1 ml-4 list-disc">
              {content.bestPractices.map((practice) => (
                <li key={practice} className="text-blue-700 text-xs">
                  {practice}
                </li>
              ))}
            </ul>
