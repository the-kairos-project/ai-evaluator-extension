...
export const getChatCompletion: GetChatCompletion = async (messages) => {
  ...
  Logger.debug(`🔍 ROUTING DECISION: Server Mode is ${useServerMode ? 'ENABLED' : 'DISABLED'}`);
  ...
  Logger.debug(`🔀 Using ${useServerMode ? 'SERVER-ROUTED' : 'DIRECT'} API call for ${selectedProvider}`);
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    Logger.error(`💥 API Error from ${selectedProvider} after ${duration}ms: ${error.message}`);
    throw error;
  }
};
