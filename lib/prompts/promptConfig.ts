...
export const getPromptSettings = (): PromptSettings => {
  const stored = globalConfig.get('promptSettings') as
    | Partial<PromptSettings>
    | undefined;

  return {
    selectedTemplate:
      stored?.selectedTemplate || DEFAULT_PROMPT_SETTINGS.selectedTemplate,
    ...
    additionalInstructions:
      stored?.additionalInstructions || DEFAULT_PROMPT_SETTINGS.additionalInstructions,
  };
};
