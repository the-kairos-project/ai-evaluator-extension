...
import { type Table, type Record as AirtableRecord } from '@airtable/blocks/models';
import { type FailedApplicant, removeFailedApplicants } from './failedApplicants';
import { type Preset } from './preset';
import { evaluateApplicants, type SetProgress } from './evaluateApplicants';
...
export const retryFailedApplicants = async (
  ...
  if (notFoundIds.length > 0) {
    setResult(`Warning: ${notFoundIds.length} applicants not found (may have been deleted)`);
      ...
      try {
        ...
        const evaluation = await Promise.race([
          ...
          new Promise<Record<string, unknown>>((_, reject) => 
            setTimeout(() => reject(new Error(`Retry timeout after ${RETRY_TIMEOUT/1000}s`)), RETRY_TIMEOUT)
          )
        ]);
        ...
        const totalTime = Date.now() - startTime;
        console.error(`❌ Failed to retry applicant ${record.id} after ${totalTime}ms:`, error);
