...
import { getChatCompletion } from "./getChatCompletion/anthropic";
...
export const evaluateApplicants = (
  ...
  console.debug(`Evaluating ${applicants.length} applicants`);
  return applicants.map(async (applicant) => {
    const innerSetProgress: SetProgress = (updater) => {
      setProgress(
        (progress) => progress + (1 / applicants.length) * updater(0),
      );
    };
...
const convertToPlainRecord = (
  ...
  preset.applicantFields.forEach((field) => {
    const questionName =
      field.questionName ??
      ((applicant as any).parentTable as AirtableTable).getFieldById(
        field.fieldId,
      ).name;
